...
// static std::string static_output;
...
int DoFetchNextSeq(const std::vector<folly::StringPiece>& command_lines) {
  LOG(INFO) << "DoFetchNextSeq..." << std::endl;
  uint32_t id = 0;
  try {
  ...
  ZRpcClientCall<zproto::SequenceRsp>(
      ...
      [](std::shared_ptr<ApiRpcOk<zproto::SequenceRsp>> seq_rsp,
        ...
        if (rpc_error) {
          ...
          static_output = seq_rsp->ToString();
  ...
  ZRpcClientCall<zproto::SequenceListRsp>(
      ...
      [](std::shared_ptr<ApiRpcOk<zproto::SequenceListRsp>> seqs_rsp,
        ...
        if (rpc_error) {
          ...
          static_output = seqs_rsp->ToString();
        }
        return 0;
      });
  return 0;
...
int DoClientCommand(const std::vector<folly::StringPiece>& command_lines) {
  ...
  LOG(INFO) << " CMD->HANDLER...";
  // return cmd->handler(command_lines,out);
  auto ret = cmd->handler(command_lines);
  std::cout << "CMD->HANDLER,OUTPUT:" << static_output << std::endl;
  return ret;
}
