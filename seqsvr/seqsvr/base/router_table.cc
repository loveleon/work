...
void RouteTable::MakeTestRouteTable(RouteTable& table) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      alloc_config.alloc_name = std::string("alloc_server:") +
                                folly::to<std::string>(alloc_config.addr.port);
...
// add by cxh
void RouteTable::MakeRouteTable(RouteTable& table,
                                std::vector<std::string>& ipvec) {
  table.set_version(Now());
  int alloc_id = 0;
  int set_id = 0;
  int port = 10000;
  std::string alloc_ip("127.0.0.1");
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    // port = 10000;  // we don't want the port++ (eg,10000 10001 10002 10003)
    // ->(10000,10001)
    for (int j = 0; j < 2; j++) {
      // for (int j = 0; j < 1; j++) {
      AllocEntry alloc_config;
      ...
      // alloc_1
      alloc_config.alloc_id = (int64_t)set_id >> 32 | alloc_id++;
      alloc_config.addr.host = alloc_ip;  //"127.0.0.1";
      alloc_config.addr.port = port++;
      alloc_config.alloc_name = std::string("alloc_server:") +
                                folly::to<std::string>(alloc_config.addr.port);
      range.id = i * 10000000 + j * 5000000;
...
AllocEntry* RouteTable::LookupAlloc(const std::string& set_name,
