...
class RouteTable {
 public:
  ...
  static void MakeRouteTable(RouteTable& table,
                             std::vector<std::string>& ipvec);

  uint32_t version() const { return version_; }

  inline void set_version(uint32_t v) { version_ = v; }
  ...
  AllocEntry* LookupAlloc(const std::string& set_name,
 ...
 private:
  ...
  uint32_t version_{0};
  // std::string name_;     //
  // 服务名，可能会有多个seqsvr提供服务，为统一管理，提供一个唯一名字
