...
/*
 ...
 2. 分配sequence时，将cur_seq++，同时与分配上限max_seq比较：
    如果cur_seq > max_seq，将分配上限提升一个步长max_seq +=
 step，并持久化max_seq
...
bool AllocSvrManager::GetCurrentSequence(uint32_t id, uint32_t client_version,
                                         SequenceWithRouterTable& o) {
...
bool AllocSvrManager::FetchNextSequence(uint32_t id, uint32_t client_version,
                                        SequenceWithRouterTable& o) {
  if (!cache_alloc_entry_) return false;
  if (state_ != kAllocInited) return false;
...
// 路由表更新
void AllocSvrManager::OnLeaseUpdated(RouteTable& table) { table_.Swap(table); }
...
void AllocSvrManager::LoadMaxSeq() {
  ...
  ZRpcClientCall<zproto::LoadMaxSeqsDataRsp>(
      "store_client", MakeRpcRequest(load_max_seqs_data_req),
      [&](std::shared_ptr<ApiRpcOk<zproto::LoadMaxSeqsDataRsp>>
...
void AllocSvrManager::SaveMaxSeq(uint32_t section_id,
                                 uint64_t section_max_seq) {
  // TODO(@beqni): NWR写
  //  auto store = StoreSvrManager::GetInstance();
  //  bool rv = store->SetSectionsData(set_id, alloc_id, section_id,
  //  section_max_seq);
  ...
  ZRpcClientCall<zproto::SaveMaxSeqRsp>(
      "store_client", MakeRpcRequest(save_max_seq_req),
      [section_max_seq, this](
          std::shared_ptr<ApiRpcOk<zproto::SaveMaxSeqRsp>> save_max_seq_rsp,
          ProtoRpcResponsePtr rpc_error) -> int {
        if (rpc_error) {
          LOG(ERROR) << "SaveMaxSeqReq - rpc_error: " << rpc_error->ToString();
          this->OnMaxSeqSaved(false);
        } else {
          LOG(INFO) << "SaveMaxSeqReq - load_max_seqs_data_rsp: "
                    << save_max_seq_rsp->ToString();
          this->OnMaxSeqSaved((*save_max_seq_rsp)->last_max_seq() ==
                              section_max_seq - 1);
        }
        return 0;
      });
...
void AllocSvrManager::OnMaxSeqLoaded(const std::string& data) {
  if (!data.empty()) {
    ...
    for (int i = 0; i < kSectionSlotSize - 1; ++i) {
      std::fill(cur_seqs_.begin() + i * kSectionSize,
                cur_seqs_.begin() + (i + 1) * kSectionSize,
                section_max_seqs_[i]);
    }
