...
BaseDaemon::~BaseDaemon() { g_main_evb = nullptr; }
...
bool BaseDaemon::LoadConfig(const std::string& config_file) {
  ...
  if (rv == false) {
    LOG(ERROR)
        << "LoadConfig - ConfigManager::Initialize() error, config_path: "
        << config_file;
  }
  return rv;
}
...
const char* GetInstanceName() { return google::ProgramInvocationShortName(); }
}
