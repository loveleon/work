...
bool BaseServer::Initialize() {
  ...
  // 初始化程序
  auto thread_groups =
      std::make_shared<ThreadGroupListManager>(thread_group_options_);
  ...
  for (auto it = services_.begin(); it != services_.end(); ++it) {
    net_engine_manager->Install(std::get<0>(*it), std::get<1>(*it),
                                std::get<2>(*it));
  }
...
bool BaseServer::Destroy() { return BaseDaemon::Destroy(); }
}
