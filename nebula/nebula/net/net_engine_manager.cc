...
void NetEngineManager::set_thread_groups(
    const std::shared_ptr<ThreadGroupListManager>& thread_groups) {
  thread_groups_ = thread_groups;
}

void NetEngineManager::Install(const std::string& name, const std::string& type,
  ...
  if (io_group) {
    if (type != "tcp_server" && type != "tcp_client" && type != "http_server" &&
        type != "rpc_server" && type != "rpc_client") {
      LOG(ERROR)
          << "Install - thread_type is conn, UNKNOWN service_type: name = "
          << name << ", type = " << type << ", proto = " << proto;
      return;
    }
  } else {
    if (type == "tcp_server" || type == "http_server" || type == "rpc_server") {
      io_group =
          thread_groups_->GetIOThreadPoolExecutor(ThreadType::CONN_ACCEPT);
      if (!io_group) {
        LOG(ERROR) << "Install - not setup tcp_server's [accept] thread_type, "
                      "service_type: name = "
                   << name << ", type = " << type << ", proto = " << proto;
        return;
      }
      ...
      if (!io_group) {
        LOG(ERROR) << "Install - not setup tcp_client's [client] thread_type, "
                      "service_type: name = "
                   << name << ", type = " << type << ", proto = " << proto;
        return;
      }
  ...
  for (auto& service_config : service_configs_) {
    ...
    if (service) {
      ...
      if (!RegisterServiceBase(service)) {
        LOG(ERROR) << "Start - RegisterServiceBase error by "
                   << service_config->ToString();
      }
    } else {
      LOG(ERROR) << "Start - CreateService error, type = "
                 << service_config->ToString();
    }
  }
...
bool NetEngineManager::Pause() { return true; }
...
// 查找分组
std::shared_ptr<ServiceBase> NetEngineManager::Lookup(
...
bool NetEngineManager::RegisterServiceBase(
    std::shared_ptr<ServiceBase> service) {
  bool rv = false;
  ...
  if (service->GetServiceType() == "tcp_client" ||
    ...
    if (it != services_.end()) {
      ...
      if (it->second->GetModuleType() == ServiceModuleType::TCP_CLIENT_GROUP &&
        ...
        LOG(INFO) << "RegisterServiceBase - client: register group's service "
                  << service->GetServiceConfig().ToString();
        it->second->AddChild(service);
        rv = true;
      } else {
        // 非组或非相同服务类型，则配置有问题
        LOG(FATAL) << "RegisterServiceBase - client: not group, duplicated "
                      "service name: "
                   << service->GetServiceConfig().ToString();
      }
    } else {
      ...
      auto tcp_client_group = ServiceSelfRegisterFactoryManager::Execute3(
          k, service->GetServiceConfig(), io_group);
      tcp_client_group->AddChild(service);

      services_.insert(
          std::make_pair(service->GetServiceName(), tcp_client_group));
    ...
    // tcp_server
    if (it == services_.end()) {
      LOG(INFO) << "RegisterServiceBase - register service: "
                << service->GetServiceConfig().ToString();
      services_.insert(std::make_pair(service->GetServiceName(), service));
      rv = true;
    } else {
      LOG(FATAL) << "RegisterServiceBase - duplicated service name: "
                 << service->GetServiceName();
    }
  } else {
    // invalid service_type
    LOG(FATAL) << "RegisterServiceBase - invalid service type: "
...
ServiceBasePtr NetEngineManager::CreateService(
